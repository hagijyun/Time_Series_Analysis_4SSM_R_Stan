// mode12-1.stan
// Model: specification (local-level model + horseshoe distribution with unknown parameters, utilizing time-varying Kalman filter)

/*
 * The contents of the forefront function block are borrowed from the following site (partly modified to avoid the latest rstan's warning)
...
transformed parameters{
  vector[t_max]                   W;  // Variance of state noise (time-varying)
  vector[t_max]                   V;  // Variance of observation noise (time-varying)
  real                      log_lik;  // Log-likelihood through all time points

  // Variance of state noise (different value is set at every time point)
  for (t in 1:t_max) {
...
model{
  ...
  /* Standard deviation of state noise (time-invariant base) and standard deviation of observation noise (time-invariant): noninformative prior distribution */
}
